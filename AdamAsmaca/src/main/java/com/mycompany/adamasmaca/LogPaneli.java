...
public class LogPaneli extends JPanel {
    ...
    private void arayuzOlustur() {
        String[] sutunlar = {"Tarih & Saat", "İşlem"};
        ...
        scrollPane.setBorder(BorderFactory.createTitledBorder("Giriş Logları"));
    ...
    public void verileriYukle() {
        model.setRowCount(0);
        List<String[]> loglar = DosyaYoneticisi.loglariOku();
        for (String[] satir : loglar) {
            model.addRow(satir);
        }
    }
    ...
    private void temizle() {
        ...
        if (girilen.equals(gercekSifre)) {
            DosyaYoneticisi.dosyaTemizle(DosyaYoneticisi.LOG_DOSYA);
            model.setRowCount(0);
